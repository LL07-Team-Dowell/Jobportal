  ...
  useEffect(() => {
    ...
    if (jobs.length === 0) {
      getJobsFromAdmin(currentUser.portfolio_info[0].org_id)
        .then((resp) => {
          const JOBS = resp.data.response.data.filter(
    ...
    <>
      <div className="viewJob-container">
        ...
        <div className="job-discription">
          <div>
            <h5>Skills:</h5> <span>{skills} </span>
          </div>
          <div>
            <h5>TimePeriod:</h5> <span>{time_interval}</span>
          </div>
          <div>
            <h5>Payment:</h5>{" "}
            <span>
              {payment} {paymentInterval ? `/ ${paymentInterval}` : ""}
            </span>
          </div>
          <div>
            <h5>Job Type:</h5> <span>{type_of_job}</span>
          </div>
          <div>
            <h5>Job Category:</h5> <span>{job_category}</span>
          </div>
          <div>
            <h5>Qualification:</h5> <span>{qualification}</span>
          </div>
          {module && (
            <div>
              <h5>Module:</h5> <span>{module}</span>
            </div>
          )}
          ...
          {technical_specification.length > 0 && (
            <>
              ...
              <ol>
                {technical_specification.map((specif, index) => (
