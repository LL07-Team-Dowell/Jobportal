...
function Home({
  ...
  useEffect(() => {
    ...
    if (
      Array.isArray(candidateJobs.appliedJobs) &&
      candidateJobs.appliedJobs.length > 0
    )
      return setLoading(false);

    getAppliedJobs(currentUser?.portfolio_info[0].org_id)
      .then((res) => {
        ...
        const userSelectedJobs = currentUserAppliedJobs.filter(
          (application) => application.status === candidateStatuses.ONBOARDING
        );
        const userShortlistedJobs = currentUserAppliedJobs.filter(
          (application) => application.status === candidateStatuses.SHORTLISTED
        );
        ...
        if (
          currentUserAppliedJobs.find(
            (application) =>
              application.status === candidateStatuses.RENEWCONTRACT
          )
        ) {
        ...
        setCandidateJobs((prevJobs) => {
          return { ...prevJobs, appliedJobs: currentUserAppliedJobs };
        });
        setLoading(false);
      })
      .catch((err) => {
          ...
          <div className="content__Wrappper">
            <div className="content__Item">
              ...
              {React.Children.toArray(
                availableJobCategories.slice(0, 1).map((category) => {
                  return (
                    <Link
                      to={`/jobs/c/${category
                        .toLocaleLowerCase()
                    ...
                    >
                      <img
                        ...
                        alt="job category"
                        data-tooltip-id={"001"}
                        data-tooltip-content={"Employee jobs"}
                      />
              ...
              {React.Children.toArray(
                availableJobCategories.slice(1, 2).map((category) => {
                  return (
                    <Link
                      to={`/jobs/c/${category
                        .toLocaleLowerCase()
                    ...
                    >
                      <img
                        ...
                        alt="job category"
                        data-tooltip-id={"002"}
                        data-tooltip-content={"Internship jobs"}
                      />
            ...
            <div className="content__Item">
              ...
              {React.Children.toArray(
                availableJobCategories.slice(2, 3).map((category) => {
                  return (
                    <Link
                      to={`/jobs/c/${category
                        .toLocaleLowerCase()
                    ...
                    >
                      <img
                        src={assets.researcher}
                        alt="job category"
                        data-tooltip-id={"003"}
                        data-tooltip-content={"Research associate jobs"}
                      />
                      <Tooltip id="003" place="center" />
                    </Link>
                  );
              ...
              {React.Children.toArray(
                availableJobCategories.slice(3, 4).map((category) => {
                  return (
                    <Link
                      to={`/jobs/c/${category
                        .toLocaleLowerCase()
                    ...
                    >
                      <img
                        src={assets.freelaner}
                        alt="job category"
                        data-tooltip-id={"004"}
                        data-tooltip-content={"Freelance jobs"}
                      />
        ...
        {isLargeScreen && (
          <aside>
            <div className="side__Content">
              ...
              <p>
                DoWell is the right place to "fail", if you are innovating !!
              </p>
            </div>
          </aside>
        )}
